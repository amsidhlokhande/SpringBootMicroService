...
public class WebFluxJwtSecurityConfig {
	...
	public WebFluxJwtSecurityConfig(AuthenticationManager authenticationManager,
			SecurityContextRepository securityContextRepository, Environment environment) {
	...
	public SecurityWebFilterChain getSecurityFilterChain(ServerHttpSecurity serverHttpSecurity) {
		...
		serverHttpSecurity.authorizeExchange().pathMatchers(properties).permitAll().pathMatchers("/users/signIn")
				...
				.authenticationEntryPoint(new HttpStatusServerEntryPoint(UNAUTHORIZED)).and().csrf().disable()
